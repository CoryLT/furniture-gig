...
-- A logged-in user can create a conversation that includes them,
-- but only if neither side has blocked the other.
...
-- Participant can send a message if they are in the conversation
...
-- Sender can update their own message (e.g., mark read state on
-- received side is handled by recipient — see below)
...
-- 3. user_blocks
-- ------------------------------------------------------------
-- A user can block another user. Block prevents new conversations
-- being created and new messages being sent in either direction.
...
-- A user can view, create, and remove their own blocks
...
-- A user can create a report and view their own reports
...
-- Admin can view + update all reports
