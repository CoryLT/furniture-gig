...
## 🚀 SOFT-LAUNCH STATUS (as of May 27, 2026)

**FlipWork is LIVE and Facebook-group ready.** First end-to-end live transaction completed successfully a few sessions ago. The most-recent session shipped the first half of an **email-notifications system** (Phase 1 plumbing only — no real-user emails fire yet) plus **trust + UX upgrades**: a Stripe-backed Verified checkmark on public profiles, a real-photo+name+badge upgrade to the flipper applicant cards, and a new `gig_application` notification type in the in-app bell. End-of-session blocker: the Resend pipeline test (`/api/email/test`) was not yet returning `"sent"` because Cory had `RESEND_API_KEY` in the wrong case (`Resend_Api_Key`) in Vercel. Next session: confirm the env var is exact-case, redeploy without cache, hit `/api/email/test` and confirm sent, then proceed to email Phases 2-4.

Critical context to know when picking up:
- **Email notifications Phase 1 foundation shipped this session** (commit `098a5c0`). Plumbing only. Three new things: `notification_preferences` table (per-user toggles), `email_log` table (dedup + audit), `lib/email.ts` Resend wrapper, `/api/email/test` admin smoke-test endpoint. Phases 2-4 still TODO. **Don't start Phase 2 until Phase 1 is confirmed working in prod via the test endpoint.** See TODO #8 for the four-phase plan + the blocker.
- **`RESEND_API_KEY` env var on Vercel: name is case-sensitive.** Cory's existing Resend integration (for Supabase signup confirmations) lives in Supabase Auth, not Vercel. We needed to add `RESEND_API_KEY` to Vercel separately for `lib/email.ts` to find it. **Worth pointing out at the start of similar setups: env var names are EXACT match, no spaces, all caps as written.**
- **Verified badge system is live on public profiles.** Purple checkmark (chosen to differentiate from Twitter/Meta blue). Auto-grants and auto-revokes via the `is_user_verified(uuid)` SQL function. Means: this user has cleared a real-money trust gate (Stripe Connect active as worker OR has paid for ≥1 gig as flipper). Only rendered in two spots right now (public profile hero, applicant cards); see TODO #10 for the remaining places to wire it.
- **Applicant cards now show photo + full name + verified badge + clickable links.** Replaces the previous "gray icon + the word Worker." Real avatar, real name resolution (full_name → first+last → @username → "Worker"), purple verified check next to name when worker has active Stripe Connect, avatar AND name clickable to `/u/[username]` in a new tab.
- **In-app notifications now alert flippers when someone applies.** New `'gig_application'` notification type with realtime delivery (existing bell subscription auto-handled the new type via DB trigger). Click-through goes to `/flipper/gigs/[gig_id]` (the applicant review page).
- **PickWorkerConfirmModal already existed locally for a session, never pushed.** Cory hit "no card prompt!" alarm mid-session; investigation showed the modal was already built and committed locally but never made it to GitHub. He did `git pull && git push` and it shipped. **Watch out for local-only commits at the start of any session.** Recommend: `git fetch origin && git log origin/main..HEAD` to detect them.
- **`flipper_profiles.profile_public = false` hides ALL flipper-side data from `/u/[username]`, including photos.** Caused a 30-minute "data loss" scare this session. The user's photos were fine — they just weren't being fetched because the flipper-profile query filters by `profile_public = true`. Toggle the flag true to bring photos back. Filed as TODO #9 to fix properly.
- **iPhone HEIC photos now work everywhere.** Previously they failed silently at multiple points (empty MIME type rejected by client checks, browser-image-compression can't decode HEIC, Sightengine can't decode HEIC). New `heic-to` library in the browser converts HEIC → JPEG before compression. All 6 upload paths use a new `isAcceptableImageFile()` helper. The 5 server API routes are unchanged on purpose — by the time files arrive server-side they're always JPEG. See "HEIC photo uploads" entry below in previous session's commits.
...
8. **Email notifications (Bucket 1 #1)** — Phase 1 foundation shipped this session (commit `098a5c0`). Plumbing only — `notification_preferences` table, `email_log` table, `lib/email.ts` Resend wrapper, `/api/email/test` smoke-test endpoint. No real-user emails fire yet. Phases left:
    - **Phase 1 finishing touches (BLOCKER on test confirmation):** Cory needs to (a) verify env var `RESEND_API_KEY` is in Vercel Production scope with exact case spelling (he hit `Resend_Api_Key` case-mismatch end-of-session), (b) redeploy without build cache so the env var is baked in, (c) hit `https://myflipwork.com/api/email/test` as admin and confirm response is `{"ok": true, "status": "sent"}` and the test email lands in inbox. Until Phase 1 is confirmed working, do NOT start Phase 2.
    - **Phase 2: Settings UI at `/profile/notifications`** — three toggle switches (picked / rejected / new message), wires to `notification_preferences` row. Link from hamburger nav.
    - **Phase 3: Wire the three event types.** "You got picked" → fire when `gig_claims.status` flips to `active`. "Your application wasn't selected" → fire when status flips to `rejected`. "New message" → fire when a new row hits `gig_messages` or `listing_messages` (debounce to avoid spam if someone sends 5 in a row). All respect `notification_preferences`. All log to `email_log`. Pick whether to do this in API routes (after Stripe-pick succeeds, after rejection action, after message send) OR via Supabase triggers + pg_net to call back into Next API — both options have trade-offs Cory hasn't decided yet.
    - **Phase 4: HTML email templates + footers.** Real branded templates (not the bare HTML used by `/api/email/test`). "Do not reply" footer with link to `/support`. Unsubscribe link in every email → `/profile/notifications`.
9. **Fix `profile_public` quirk on `/u/[username]`** — Discovered during the work-samples false-alarm. Line 24 of `app/u/[username]/page.tsx` filters `flipper_profiles` by `profile_public = true`. If a user has BOTH worker + flipper profiles and toggles their flipper-public off, the entire flipper side disappears from their public profile — INCLUDING the photos (which the merged profile is responsible for showing). Photos and other shared data should show as long as EITHER profile is public. Fix: query both profiles without the `profile_public` filter on at least one side, OR compute "is this user discoverable at all" from `worker.profile_public OR flipper.profile_public`. Took 30 minutes to diagnose because it looked exactly like data loss. Worth a fix so it doesn't trip anyone else.
10. **Wire `VerifiedBadge` into more spots.** Currently only shown on `/u/[username]` (hero card) and `/flipper/gigs/[id]` (applicant cards). Other high-trust spots to add it: marketplace listing cards (seller name), chat headers (`/messages/[id]` and the listing-chat equivalent), gig cards on `/gigs` browse page (poster name), gig detail page `/gigs/[slug]` (poster sidebar). Each spot needs the relevant query to pull `is_user_verified()` or the appropriate Stripe/payment flags. Use the existing `(supabase.rpc as any)('is_user_verified', { target_user_id: id })` pattern.
11. **Address/pickup details on gigs (Bucket 1 #3).** Gigs only have city/state. Want full address visible to picked worker only. Schema change + reveal-after-pick UI.
12. **Apply `force-dynamic` audit to other server pages.** The flipper dashboard, gig detail pages, AND `/my-gigs` (added this session) all have `force-dynamic + revalidate=0` now. Still worth scanning other server pages that show claim/applicant/message state (`/messages`, `/messages/[id]`, `/home`, etc.) and adding the same if they exhibit similar staleness.
13. **`types/database.ts` is out of sync** with several Stripe columns AND the new `listing_conversations`/`listing_messages`/`listing_reports` tables. Adding all of them would let us remove the `as any` casts sprinkled through all Stripe-touching AND listing-messaging files.
14. **Mutual cancel flow for gigs (and tighten hard delete).** ⚠️ Known safety hole as of this session: the new `/api/gigs/[id]/delete` blocks delete on gigs where Stripe money has moved, but it does NOT block delete on gigs that have a claimed/active/pending worker without payment yet. Cory test-deleted a claimed gig and that's how this came up. He paused building the fix and wants it on the list.
...
## This session's commits (most recent first) — May 26-27, 2026 evening

- `098a5c0` Email notifications Phase 1: foundation (SQL + Resend wrapper). **Plumbing only — no real-user emails fire yet.** Three pieces: (a) `supabase/schema_email_notifications_phase1_20260526.sql` adds `notification_preferences` (one row per user, three boolean toggles defaulting ON: `email_picked`, `email_rejected`, `email_messages`; client RLS allows read+update of own row; AFTER INSERT trigger on `users` auto-creates a default row; existing users backfilled) plus `email_log` (records every send attempt with `status` in `sent`/`failed`/`skipped_preferences`/`skipped_duplicate`, UNIQUE partial index on `idempotency_key WHERE NOT NULL` so dedup keys are unique but failures don't claim a slot). (b) `lib/email.ts` wraps the Resend SDK. Single `sendEmail()` entry point that runs idempotency check → preferences check → Resend call → logs the result. From: `FlipWork <notifications@myflipwork.com>`. No reply-to (Phase 4 will footer-link to `/support`). NEVER throws — failures log and return a result object so app actions don't break on Resend hiccups. Uses service-role Supabase client (regular client can't write `email_log`). (c) `app/api/email/test/route.ts` admin-only smoke test endpoint that sends one test email and returns the full `sendEmail()` result as JSON. Hit `GET /api/email/test` as admin to verify pipeline. New dep: `resend@^6.12.4`.
- `2507e88` Applicant cards: real photo, full name, clickable, verified badge. The card on `/flipper/gigs/[id]` previously showed a gray person icon and the word "Worker" when first/last name was missing. Now: 44px round `next/image` avatar from `worker_profiles.avatar_url`, falls back to gray-circle initials, falls back to the User icon only as last resort. Display name tries `full_name` first (newer column the unified profile saver writes), then `first_name + last_name`, then `@username`, then "Worker". Purple `VerifiedBadge` next to the name when `stripe_charges_enabled AND stripe_payouts_enabled` (worker-side Stripe-active = flipper is about to send money to that Stripe Connect account, so it's the right trust signal here). Avatar AND name are now clickable Links to `/u/[username]` (target=_blank). Small "View profile" link kept in the corner for clarity. Added `full_name`, `avatar_url`, `stripe_charges_enabled`, `stripe_payouts_enabled` to the `worker_profiles` select. No SQL — all data already existed.
- `093f715` Notifications: alert the flipper when a worker applies to their gig. New SQL migration `supabase/schema_notifications_gig_application_20260526.sql` extends `notifications.type` CHECK to include `'gig_application'` and adds a SECURITY DEFINER trigger `create_gig_application_notification` on `gig_claims` (AFTER INSERT OR UPDATE OF status). Fires only when row enters `'pending'` state (skips spurious updates where status was already pending; skips self-applications and missing-gig edge cases). Payload `data` includes `claim_id`, `gig_id`, `gig_slug`, `gig_title`. Frontend changes to `NotificationBell.tsx`: widened `type` union to `'follow' | 'gig_application'`, added per-type `clickHref` so application notifs link to `/flipper/gigs/[gig_id]` (the applicant review page) instead of the actor's profile, new `renderMessage` case "**Worker** applied to **Gig Title**." (existing realtime subscription auto-handles new types — no subscription changes). Existing pending claims are NOT backfilled; only future applications fire notifs.
- `f9127dd` Verified badge: purple instead of blue, drop "Verified" text pill. Swapped `fill-blue-500`/`text-blue-700` → `fill-purple-500`/`text-purple-700` in `VerifiedBadge.tsx` to differentiate from Twitter/Meta blue. Removed the "Verified" text pill from the public profile hero card — checkmark next to the name is enough (the hover tooltip still explains it). Founding Member pill kept. `VerifiedBadge` component still supports `withLabel={true}` for any future use.
- `2075757` Add Verified user badge (Stripe-backed) on public profile. **First spot only — more places coming.** Two new files: `supabase/schema_verified_users_20260526.sql` (creates SECURITY DEFINER `is_user_verified(uuid)` RPC granting EXECUTE to `anon, authenticated`; returns true if either (a) `worker_profiles.stripe_charges_enabled AND stripe_payouts_enabled` for the user OR (b) at least one `payout_records.flipper_user_id` row with `payment_status IN ('captured','transferred','refunded')`) and `components/shared/VerifiedBadge.tsx` (small reusable component, three sizes sm/md/lg, icon-only or icon+label modes, uses lucide `BadgeCheck`). Wired into `app/u/[username]/page.tsx` (calls RPC alongside other parallel fetches, with `(supabase.rpc as any)` cast for known Supabase types-out-of-sync issue) and `components/profile/PublicProfileClient.tsx` (renders inline next to h1 name).

### What got accomplished this session beyond the commits

- **Verified-user system is live on public profiles.** Earns automatically — no user action needed. Cleans up over time as people's Stripe accounts change state. Purple checkmark (not blue) to differentiate from Twitter/Meta. Anonymous viewers see it too.
- **Notification bell has its first business-event notification.** Beyond the "follow" event that already existed, the bell now alerts flippers in real-time when someone applies to their gig. The bell's realtime subscription was already future-proofed for new types — only the type union, render case, and click-through href needed code changes.
- **Applicant review experience is dramatically better.** Photo + name + verified check + clickable everywhere. Flippers can actually scope out who's applying before picking instead of squinting at "Worker" + a gray icon.
- **Big false-alarm scare debugged.** Cory said his work samples got removed by Claude's verified-badge changes. After investigation: work samples were never actually deleted. Two things converged: (1) admin account `corythacker@proton.me` has no profile rows at all, so its `/u/...` would render nothing — but that wasn't the profile Cory was looking at, (2) Cory's actual user account is `corythacker@gmail.com` at `/u/corythacker`, which has 6 flipper photos in the DB. The page showed "No photos yet" because `flipper_profiles.profile_public = false` — line 24 in `app/u/[username]/page.tsx` filters the flipper-profile query by `profile_public = true`, so when that flag is false the entire flipper side (including its photos) gets hidden. Toggling `profile_public` true brought the photos back. **Known design quirk worth fixing later**: if a user has BOTH worker and flipper profiles and turns off flipper-public, their flipper-side photos disappear from `/u/...` even though their worker profile is still public. Photos should show as long as EITHER profile is public. Filed as a future cleanup, not done in this session. See "TODOs" below.
- **Other false-alarm debugged: pick-worker not asking for card.** Cory picked a worker and noticed no card-entry prompt appeared. Investigation showed `payout_records.payment_status = 'authorized'` for that pick — i.e. the system DID place a hold; it just used the saved card silently. He asked for an explicit confirmation modal showing the card last-4 + amount before holding money. Turned out the modal **already existed locally** in commit `5440011` ("Pick worker: explicit confirm modal before money is held on saved card") but had never been pushed to GitHub from a previous session. Cory did `git pull && git push` and the modal shipped. **Watch out for this pattern: local commits stranded on Cory's machine.** Recommend doing `git fetch origin && git log origin/main..HEAD` at the start of every session to catch any local-only work.
- **Email notifications Phase 1 plumbing complete; Phases 2-4 pending.** End-of-session blocker: Cory hit a Vercel env var case-sensitivity issue when adding `RESEND_API_KEY`. He had used mixed case (`Resend_Api_Key`), which env-var-name matching is strict about. We resolved the diagnosis but had not yet confirmed the test endpoint returns `"sent"`. Picking up the next session starts at finishing the env var rename + redeploy without cache + hitting `/api/email/test` to confirm sent before moving to Phase 2.

### Lessons learned this session

**Investigate before you restore.** Cory wanted to do a Supabase backup restore the moment work samples appeared "missing" — which would have rolled back hours of real activity on a live platform. Pushing back hard, asking for one SQL query first, and walking through three rounds of checks revealed the actual cause (a privacy toggle, no data lost). Default to investigation, restore only when investigation confirms data loss with no other recovery path. The cost of a 30-second SQL check is ~zero; the cost of an unnecessary restore is hours of customer activity.

**A user signing in from a different account is a real failure mode.** Cory hitting `/api/email/test` from his non-admin account returned a confusing "Forbidden" response. The admin-only check is correct; the gotcha is that humans regularly forget which account they're logged into. For admin-only endpoints, consider adding the current user's email to the 403 response so they can see "you're logged in as `foo@bar.com`, which is not an admin" instead of guessing why.

**"Verified" labels backfire when they don't mean something.** Cory's first instinct was a self-clickable "Verify yourself" button. Pushed back: a checkmark that proves nothing is worse than no checkmark, because it erodes trust once savvy users notice. The Stripe-backed approach earns automatically off real-money trust gates and means something concrete. Don't ship trust signals that can be self-issued without verifying something hard.

**Env var names are case-sensitive AND space-sensitive on Vercel.** Bit Cory at end-of-session. `Resend_Api_Key` ≠ `RESEND_API_KEY` ≠ `RESEND_API_KEY ` (trailing space). When `lib/email.ts` couldn't find the key, the diagnostic was good ("Resend not configured" returned from the helper) but the user-facing fix took several rounds. Worth telling Cory upfront: env var names are exact-match.

---

## Previous session's commits — May 25-26, 2026 supply-side polish
